...
(function () {
  ...
  UI.monsterCard=function(m,interactive) {
    ...
    return '<'+tag+' class="monster-card" style="--rarity:'+rarity.color+'"'+(interactive?' data-monster="'+m.uid+'"':'')+'>'+
      (m.party!==null?'<span class="party-badge">파티</span>':'')+'<img src="'+species.art+'" alt=""><strong>'+species.name+'</strong><small>'+rarity.name+' · Lv. '+UI.fmt(m.level)+'</small></'+tag+'>';
  };
  UI.renderMonsters=function(state) {
    ...
    content.innerHTML='<p class="sheet-intro">함께 싸우고, 함께 성장합니다.<br>보유 '+UI.fmt(state.roster.length)+' / '+UI.fmt(DATA.rosterCap)+' · 파티 '+UI.fmt(count)+' / '+UI.fmt(Game.partySlots())+'<br>조련사 랭크 '+[3,6,10].map(UI.fmt).join(' / ')+'에 파티가 확장됩니다.</p><div class="roster-grid">'+state.roster.map(function(u){return UI.monsterCard(u,true);}).join('')+'</div>';
  ...
  function detail(m,state) {
    ...
    var selected=state.roster.filter(function(u){return u.party!==null;}).length, blocked=m.party===null?selected>=Game.partySlots():selected===1;
