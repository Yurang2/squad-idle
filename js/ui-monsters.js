...
(function () {
  ...
  UI.monsterCard=function(m,interactive) {
    ...
    return '<'+tag+' class="monster-card evo-'+m.evo+'" style="--rarity:'+rarity.color+'"'+(interactive?' data-monster="'+m.uid+'"':'')+'>'+
      (m.party!==null?'<span class="party-badge">파티</span>':'')+'<img src="'+species.art+'" alt=""><strong>'+species.name+(m.evo>1?' '+UI.evoLabel(m.evo):'')+'</strong><small>'+rarity.name+' · Lv. '+UI.fmt(m.level)+(m.enhance?' +'+UI.fmt(m.enhance):'')+(m.locked?' · 잠금':'')+'</small></'+tag+'>';
  };
